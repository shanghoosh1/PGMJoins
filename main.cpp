...
//    qy.attAliases= {{"c1",{{"custkey","custkey1"}}},{"custkey2","custkey"},{"orderkey1","orderkey"},{"orderkey2","orderkey"},{"linenumber1","linenumber"},{"linenumber2","linenumber"}};// only for duplicated atts   
    ...
    Query qyyy;
    qyyy.tables={{"customer",fileAdd+"customer.csv"},{"orders",fileAdd+"orders.csv"},{"lineitem",fileAdd+"lineitem.csv"},{"supplier",fileAdd+"supplier.csv"}};
...
//qyyy.eliminationOrder={{"orderkey1",1},{"orderkey2",3},{"custkey1",0},{"custkey2",4},{"partkey",2},{"nationkey",5}};
   ...
   ////////////////////////////////////////////////////////////////// Query SSS on Twitter //
    ...
    int alg=0; // to solve cycle problem// 0. main algorithm with triangulation. 1. removing a join condition 2. removing the attributes.
    ...
    //the building step is not complete. 
    //Currently, we do not use a separate class per table to keep it simple to run on different databases
    //After reading the data, we need to copy the freqs into the graph which takes time
    pgm.buildGraph(alg);
    cout<<"Time for the building graph is:\t"<<pgm.timing_seconds.readAndMakeFreqs<<endl;
    cout<<"Building the graph finished"<<endl;cout.flush();
    pgm.inferDP();
    cout<<"Inference finished"<<endl;cout.flush();
    
    if(cmdOptionExists(argv, argv+argc, "-sample"))
    {
        
        // warming up
        pgm.query.sample_sizes[0]=1000;
